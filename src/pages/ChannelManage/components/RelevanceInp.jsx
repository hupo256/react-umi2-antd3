...
export default class RelevanceInp extends Component {
  ...
  tabChange = key => {
    this.setState(
      ...
      () => {
        this.props.form.setFieldsValue({
          relatedPage: this.state.currentSelectRelatedPageOpt
            ?.slice(0, +key)
            .map(item => item.name)
            .join(' / '),
        });
  ...
  toggleSelectPanlHandle = isShow => {
    const { currentSelectRelatedPageOpt } = this.state;
    if (
      (currentSelectRelatedPageOpt[1]?.linkType === 2 && !!!currentSelectRelatedPageOpt[2]) ||
      (currentSelectRelatedPageOpt[0]?.linkType === 2 && !!!currentSelectRelatedPageOpt[1])
    ) {
      this.setState(
        prevState => {
          return {
            currentSelectRelatedPageOpt: prevState.currentSelectRelatedPageOpt.slice(
              0,
              prevState.currentSelectRelatedPageOpt.length - 1
            ),
          };
        },
        () => {
          this.props.form.setFieldsValue({
            relatedPage: this.state.currentSelectRelatedPageOpt
              ?.slice(0, this.state.currentSelectRelatedPageOpt.length)
              .map(item => item.name)
              .join(' / '),
          });
        }
    ...
    return (
      ...
      <Form style={{ width: '100%' }}>
        <Form.Item>
          ...
          {showSelectPanl && (
            <div ref="parentNode" className={styles['card-container']}>
              <Tabs type="card" tabBarGutter={0} activeKey={currentKey} onChange={this.tabChange}>
                <TabPane tab={currentSelectRelatedPageOpt[0]?.name || '请选择'} key="0">
                  {relatedPageOption?.map(item => (
                    <p
                      ...
                      key={item.uid}
                      onClick={() => this.selectedHandle(item, '0')}
                    >
                      {item.name}
                    </p>
                  ))}
                  ...
                  <TabPane tab={currentSelectRelatedPageOpt[1]?.name || '请选择'} key="1">
                    {currentSelectRelatedPageOpt[0].children.map(item => (
                      <p
                        style={{ cursor: 'pointer' }}
                        key={item.uid}
                        onClick={() => this.selectedHandle(item, '1')}
                      >
                        {item.name}
                      </p>
                    ))}
                ...
                {(currentSelectRelatedPageOpt[1]?.linkType === 2 ||
                  currentSelectRelatedPageOpt[0]?.linkType === 2) && (
                  <TabPane
                    tab="请选择"
                    key={currentSelectRelatedPageOpt[0]?.linkType === 2 ? '1' : '2'}
                  >
                    <Search
                      value={searchText}
                      ...
                      onChange={e => {
                        ...
                        this.setState({ searchText: value, pageNum: 1 });
                    ...
                    {detailType === 4 && (
                      <Radio.Group
                        ...
                        buttonStyle="solid"
                        size="small"
                        value={currentarticleDicCode}
                        buttonStyle="solid"
                        onChange={this.radioGroupChange}
                      >
                        {articleDicOpts.map(item => (
