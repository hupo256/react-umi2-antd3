 ...
 * @Last Modified time: 2020-12-30 19:06:17
 * 文件上传
 */
...
class DraggerUpload extends Component {
  ...
  beforeUpload = (file, subType, type, bsId, f) => {
    ...
    const { long, name } = this.props;
    if (!isLt2M) {
      message.warning('单个文件仅支持300M内!', 2);
      return false;
    } else if (this.state[name].length >= (long || 50)) {
      message.warning('最多可上传50个文件哦~', 2);
      return false;
  ...
  handlePreview = ({ uid }, name) => {
    ...
    for (var i = 0; i < _arr.length; i++) {
      if (_arr[i].uid == uid) {
        previewAll(_arr[i].response.data);
      }
    }
  };
