/*
 * @Author: zqm 
 * @Date: 2020-12-07 16:19:49 
 * @Last Modified by: zqm
 * @Last Modified time: 2021-01-22 14:26:17
 * 人员选择
 */

import React, { Component } from 'react';
import { Button, Drawer, Input, Row, Col, Divider, Table, message, Checkbox } from 'antd';
import { connect } from 'dva';

@connect(({ base, loading }) => ({
  base,
  Loading: loading.effects['base/queryPersonnelModel'],
}))
class PersonnelDrawer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      inputVal: '',
    };
  }
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'base/queryPersonnelModel',
      payload: { pageNum: 1 },
    });
  }
  render() {
    const {
      title,
      visible,
      type,
      onClose,
      onDrawerSearch,
      selectedRowKeys,
      onRowChange,
      Loading,
      base: { personnelList, personnelSearch },
    } = this.props;

    const columns = [
      {
        title: '',
        dataIndex: 'checkbox',
        key: 'checkbox',
        render: (t, r) => {
          const checked = selectedRowKeys.map(item => item.uid).includes(r.uid);
          return (
            <Checkbox
              disabled={checked && this.props.disabled}
              checked={checked}
              onChange={() => onRowChange(type, 'select', checked, r)}
            />
          );
        },
      },
      {
        title: '姓名',
        dataIndex: 'realName',
        key: 'realName',
      },
      {
        title: '手机号码',
        dataIndex: 'mobile',
        key: 'mobile',
      },
      {
        title: '部门',
        dataIndex: 'deptName',
        key: 'deptName',
        render: text => {
          return text || '/';
        },
      },
      {
        title: '职务',
        dataIndex: 'position',
        key: 'position',
        render: text => {
          return text || '/';
        },
      },
    ];

    return (
      <div>
        <Drawer
          title={title || '选择工程内部主负责人'}
          placement="right"
          closable={true}
          onClose={() => {
            this.setState({ inputVal: '' });
            this.props.dispatch({
              type: 'base/resetModel',
              payload: { personnelSearch: {} },
            });
            onClose();
          }}
          visible={visible}
          width="600"
        >
          <Row>
            <Col span={12}>
              <Input
                placeholder={'请输入姓名'}
                value={this.state.inputVal}
                onChange={e => {
                  this.setState({ inputVal: e.target.value });
                  this.inputChange(e);
                }}
                style={{ width: 200 }}
              />
            </Col>
            <Col span={12}>
              <Button
                onClick={() => onDrawerSearch(personnelSearch.userName)}
                type="primary"
                style={{ display: 'none' }}
              >
                查询
              </Button>
            </Col>
          </Row>
          <Divider />
          <Table
            columns={columns}
            dataSource={personnelList && personnelList.list}
            loading={Loading}
            rowKey={'uid'}
            pagination={{
              pageSize: 10,
              hideOnSinglePage: true,
              current: personnelList && personnelList.curPage,
              total: personnelList && personnelList.recordTotal,
            }}
            onChange={this.handleTableChange}
          />
        </Drawer>
      </div>
    );
  }
  inputChange = e => {
    this.loadQuery({ userName: e.target.value, pageNum: 1 });
  };
  loadQuery = obj => {
    const {
      dispatch,
      base: { personnelSearch },
    } = this.props;
    dispatch({
      type: 'base/queryPersonnelModel',
      payload: { ...personnelSearch, ...obj },
    });
  };
  // 分页
  handleTableChange = pagination => {
    this.loadQuery({ pageNum: pagination.current, pageSize: pagination.pageSize });
  };
}

export default PersonnelDrawer;
